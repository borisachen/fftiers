...
download.data <- function(pos.list=c('rb','wr','te','flx'), scoring='STD') {
	...
	if (download == TRUE) {
		for (mp in pos.list) {
			Sys.sleep(1)
			position = toupper(mp)
		 	#rmold1 = paste('rm ~/projects/fftiers/dat/2024/week-', thisweek, '-',mp,'-raw.txt', sep='')
		 	#system(rmold1)
		 	if (thisweek == 0)
		 	...
		 	if (thisweek != 0)
		  	...
		  	head.dir = '~/projects/fftiers/dat/2024/week-'
...
error.bar.plot <- function(pos="NA", low=1, high=24, k=8, format="NA", title="dummy", tpos="QB", dat,
	...
	if ((thisweek==0) && (tpos!='ALL')) title = paste("2024 Draft - ",tpos," Tiers", ' - ', curr.time, ' PST', sep="")
	if ((thisweek==0) && (tpos=='ALL')) title = paste("2024 Draft - Top 200 Tiers", ' - ', curr.time, ' PST', sep="")
	#dat$Rank = 1:nrow(dat)
	this.pos = dat
