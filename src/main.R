...
error.bar.plot <- function(pos="NA", low=1, high=24, k=8, format="NA", title="dummy", tpos="QB", dat, adjust=0) {
	#title = paste("Week ",thisweek," - ",tpos," Tiers", sep="")
	title = paste("Pre-draft 7/20/14 - ",tpos," Tiers", sep="")
	#colnames(dat) = c(colnames(dat)[2:8], "Rank")
	dat$Rank = 1:nrow(dat)
	this.pos = dat
	...
	#### EXPERIMENTAL
	#this.pos$Tier	= as.character(as.numeric(as.character(this.pos$mcluster))+adjust)
	bigfont			= c("QB","TE","K","DST", "PPR - TE", "ROS - TE", "0.5 PPR - TE", "ROS - QB")
	smfont			= c("RB", "PPR - RB", "ROS - RB", "0.5 PPR - RB")
	tinyfont		= c("WR","Flex", "PPR - WR", "ROS - WR","PPR - Flex", "0.5 PPR - WR","0.5 PPR - Flex")
	...
	if (tpos %in% "ALL")   {font = 2.5; barsize=1;    dotsize=1;   }
    ...
    p = p + coord_flip()
#    if (adjust>0) p = p + scale_shape_discrete(labels=c(adjust:(adjust+4)))
    ...
    if (tpos == 'ALL')     			
    	p = p + geom_text(aes(label=Player.Name, colour=Position, y = Ave.Rank - nchar/3 - Std.Dev/1.8), size=font) 
    ...
    if (tpos != 'ALL') p = p + scale_colour_hue(l=55, h=c(0,300))
    if (tpos == 'ALL') p = p + scale_colour_hue(l=55, h=c(0,400))
    maxy = max( abs(this.pos$Ave.Rank)+this.pos$Std.Dev/2) 
	if (tpos!='Flex') p = p + ylim(-4, maxy)
    if (tpos=="Flex") p = p + ylim(4, maxy)
	if (tpos=='ALL') p = p + ylim(low-6, maxy)
	...
	if (useold == TRUE) {
	  ...
	  p = p + ggtitle(title)
      p = p + geom_errorbar(aes(ymin=Ave.Rank-Std.Dev/2, ymax= Ave.Rank + Std.Dev/2, 
    		width=0.2), colour="grey80")
  	  p = p + geom_point(colour="grey20", size=dotsize, alpha=0.5) 
      p = p + annotate("text", x = Inf, y = -Inf, label = "www.borischen.co", hjust=1.1, 
    		vjust=-1.1, col="white", cex=6, fontface = "bold", alpha = 0.8)
	  if (tpos %in% bigfont)     			
    	p = p + geom_text(aes(label=Player.Name, colour=factor(mcluster), y = Ave.Rank), size=font, angle=15) 
	  if (tpos %in% smfont)     			
    	p = p + geom_text(aes(label=Player.Name, colour=factor(mcluster), y = Ave.Rank), size=font, angle=15) 
	  if (tpos %in% tinyfont)     			
    	p = p + geom_text(aes(label=Player.Name, colour=factor(mcluster), y = Ave.Rank), size=font, angle=15) 
...
draw.tiers <- function(pos, low, high, k, adjust=0) {
	...
	error.bar.plot(low = low, high = high, k=k, tpos=tpos, dat=dat, adjust=adjust)
}
pos='qb'
injured <- c()

draw.tiers("all", 1, 50, 5)
draw.tiers("all", 50, 100, 5, adjust=5)
...
dat = read.delim(paste(datdir, "week_",thisweek,"_ros-qb.tsv",sep=""), sep="\t")
error.bar.plot(low = 1, high = 30, k=6, tpos="ROS - QB")
dat = read.delim(paste(datdir, "week_",thisweek,"_ros-rb.tsv",sep=""), sep="\t")
error.bar.plot(low = 1, high = 40, k=8, tpos="ROS - RB")
dat = read.delim(paste(datdir, "week_",thisweek,"_ros-wr.tsv",sep=""), sep="\t")
error.bar.plot(low = 1, high = 60, k=7, tpos="ROS - WR")
dat = read.delim(paste(datdir, "week_",thisweek,"_ros-te.tsv",sep=""), sep="\t")
error.bar.plot(low = 1, high = 30, k=5, tpos="ROS - TE")
dat = read.delim(paste(datdir, "week_",thisweek,"_ros-k",sep=""), sep="\t")
error.bar.plot(low = 1, high = 80, k=11, tpos="ROS - K")
